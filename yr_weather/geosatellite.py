...
class Geosatellite(BaseClient):
    ...
    def get_image(
        ...
        if area not in areaArgs:
            raise ValueError(
                f"The 'area' parameter must be one of the possible SatAreas: {areaArgs}"
            )

        if img_type not in typeArgs:
            raise ValueError(
                f"The 'img_type' parameter must be one of the possible image types: {typeArgs}"
            )

        if size not in sizeArgs:
            raise ValueError(
                f"The 'size' parameter must be one of the possible sizes: {sizeArgs}"
            )
